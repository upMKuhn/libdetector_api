...
const LibraryDetectorTests = {
                ...
                bootstrapComponents.some(function (component) {
                    if (win.$.fn[component]) {
                        ...
                        if (win.$.fn[component].Constructor && win.$.fn[component].Constructor.VERSION) {
                            ...
                            new RegExp(RE_PREFIX_V3 + component).test(win.$.fn[component].toString())
                        ) {
                            bootstrapVersion = '>= 3.0.0 & <= 3.1.1'
                            return true
                            // Bootstrap < 3.1.0 detection
                        } else if (
                            new RegExp(RE_PREFIX_V2 + component).test(win.$.fn[component].toString())
                        ) {
                ...
                var d41d8cd98f00b204e9800998ecf8427e_LibraryDetectorTests = {
                            ...
                            if (jQueryAvailable) {
                                ...
                                bootstrapComponents.some(function (component) {
                                    if (win.$.fn[component]) {
                                        // Bootstrap >= 3.2.0 detection
                                        if (
                                            win.$.fn[component].Constructor &&
                                            win.$.fn[component].Constructor.VERSION
                                        ) {
                                            bootstrapVersion = win.$.fn[component].Constructor.VERSION
                                            return true
                                            // Bootstrap >= 2.0.0 and <= 3.1.0 detection
                    ...
                    React: {
                        ...
                        test: function (win) {
                            ...
                            var reactRoot = win.document.getElementById('react-root')
                            var altHasReact = win.document.querySelector('*[data-reactroot]')
                            var bodyReactRoot =
                                isMatch(win.document.body) || isMatch(win.document.body.firstElementChild)
                            var hasReactRoot =
                                bodyReactRoot ||
                                win.document
                                    .createTreeWalker(win.document.body, NodeFilter.SHOW_ELEMENT, nodeFilter)
                                    .nextNode() != null
                            if (
                                hasReactRoot ||
                    ...
                    Preact: {
                        ...
                        test: function (win) {
                            ...
                            var preactRoot =
                                getMatch(win.document.body) || getMatch(win.document.body.firstElementChild)
                            if (!preactRoot) {
                                preactRoot = win.document
                                    .createTreeWalker(win.document.body, NodeFilter.SHOW_ELEMENT, nodeFilter)
                                    .nextNode()
                            }
                    ...
                    Vue: {
                        ...
                        test: function (win) {
                            ...
                            var hasVueNode =
                                win.document
                                    .createTreeWalker(win.document.body, NodeFilter.SHOW_ELEMENT, isVueNode)
                                    .nextNode() !== null
                    ...
                    Marko: {
                        ...
                        test: function (win) {
                            ...
                            var markoElement = win.document.querySelector(selector)
                            if (markoElement) {
                                return { version: UNKNOWN_VERSION }
                            }
                            return false
                    ...
                    Gatsby: {
                        ...
                        npm: 'gatsby',
                        test: function (win) {
                            if (win.document.getElementById('___gatsby')) {
                                return { version: UNKNOWN_VERSION }
                            }
                            return false
                        },
                    ...
                    WordPress: {
                        ...
                        npm: null,
                        test: function (win) {
                            const hasAPILinkElem = !!win.document.querySelector('link[rel="https://api.w.org/"]')
                            const hasWPIncludes = !!win.document.querySelectorAll(
                    ...
                    Workbox: {
                        ...
                        test: async function (win) {
                            ...
                            const workerPromise = nav.serviceWorker
                                .getRegistration()
                                .then(function (registration) {
                                    var scriptURL = nav.serviceWorker.controller
                                        ? nav.serviceWorker.controller.scriptURL
                            ...
                            if (shared) {
                                ...
                                return {
                                    version: Array.isArray(versions)
                                        ? versions.map((it) => `core-js-${it.mode}@${it.version}`).join('; ')
                                        : UNKNOWN_VERSION,
                                }
                            } else if (core) {
                    ...
                    Drupal: {
                        ...
                        test: function (win) {
                            ...
                            const res = Array.from(win.document.querySelectorAll('link,style,script') || [])
                    ...
                    'Create React App': {
                        ...
                        test: function (win) {
                            ...
                            if (
                                root &&
                                noscript &&
                                /You need to enable JavaScript to run this app/.test(noscript.textContent)
                            ) {
                                return { version: UNKNOWN_VERSION }
                            }
    ...
    React: {
        ...
        test: function (win) {
            ...
            var reactRoot = win.document.getElementById('react-root')
            var altHasReact = win.document.querySelector('*[data-reactroot]')
            var bodyReactRoot = isMatch(win.document.body) || isMatch(win.document.body.firstElementChild)
            var hasReactRoot =
                bodyReactRoot ||
                win.document.createTreeWalker(win.document.body, NodeFilter.SHOW_ELEMENT, nodeFilter).nextNode() !=
                    null
            if (
                hasReactRoot ||
                (reactRoot && reactRoot.innerText.length > 0) ||
    ...
    'React (Fast path)': {
        ...
        test: function (win) {
            ...
            var reactRoot = win.document.getElementById('react-root')
            var altHasReact = win.document.querySelector('*[data-reactroot]')
            var hasReactRoot = isMatch(win.document.body) || isMatch(win.document.body.firstElementChild)
            if (hasReactRoot || reactRoot || altHasReact || win.React) {
                return { version: (win.React && win.React.version) || UNKNOWN_VERSION }
            }
            return false
    ...
    Preact: {
        ...
        test: function (win) {
            ...
            var preactRoot = getMatch(win.document.body) || getMatch(win.document.body.firstElementChild)
            if (!preactRoot) {
                preactRoot = win.document
                    .createTreeWalker(win.document.body, NodeFilter.SHOW_ELEMENT, nodeFilter)
                    .nextNode()
    ...
    'Velocity.js': {
        ...
        test: function (win) {
            ...
            if (velocity && velocity.RegisterEffect && velocity.version) {
                return {
                    version:
                        velocity.version.major + '.' + velocity.version.minor + '.' + velocity.version.patch,
                }
            } else if (velocity && velocity.RegisterEffect) {
                return { version: UNKNOWN_VERSION }
            }
    ...
    Vue: {
        ...
        test: function (win) {
            ...
            var hasVueNode =
                win.document.createTreeWalker(win.document.body, NodeFilter.SHOW_ELEMENT, isVueNode).nextNode() !==
                null
            if (hasVueNode || win.Vue) {
                return { version: (win.Vue && win.Vue.version) || UNKNOWN_VERSION }
            }
    ...
    Workbox: {
        ...
        test: async function (win) {
            ...
            const workerPromise = nav.serviceWorker.getRegistration().then(function (registration) {
                var scriptURL = nav.serviceWorker.controller
                    ? nav.serviceWorker.controller.scriptURL
                    : registration.active.scriptURL
                return fetch(scriptURL, { credentials: 'include', headers: { 'service-worker': 'script' } })
                    .then(function (response) {
                        return response.text()
                    })
                    .then(function (scriptContent) {
    ...
    Drupal: {
        ...
        npm: null,
        test: function (win) {
            const generatorMeta = win.document.querySelector('meta[name="generator"][content^="Drupal"]')
            const version = generatorMeta
                ? generatorMeta.getAttribute('content').replace(/\D+/gi, '')
                : UNKNOWN_VERSION
    ...
    TYPO3: {
        ...
        npm: null,
        test: function (win) {
            const generatorMeta = win.document.querySelector('meta[name="generator"][content^="TYPO3"]')
...
module.exports = { UNKNOWN_VERSION, LibraryDetectorTests }
